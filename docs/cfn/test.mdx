...
インフラを CloudFormation のテンプレートとして記述することができるようになり、デプロイする前からある程度インフラの品質を保証できるようになりました。
強力でバランスがとれた自動テストを導入して、インフラリソースを継続的にデリバリーする準備を整えましょう。

この章で紹介する CloudFormation におけるテストの方法は以下の３点です。

1. **Linter による静的構文解析** <br/>
   YAML または JSON で記述されたテンプレートファイルが決められたポリシーに従っているかを検証する。
...
もちろん CloudFormation を使用しても EC2 の UserData を記述することで、OS/ミドルウェア の設定も行うことができます。ただ、OS 内部の設定の場合は、Ansible や Chef などのツールを使用する方がより適切な場合が多いでしょう。
...
<img
  width="100px"
  src="https://github.com/aws-cloudformation/cfn-python-lint/raw/master/logo.png?raw=true"
  alt="cfn-python-lint"
/>
...
cfn-python-lint は コマンドラインで実行しても有効ですが、各種エディタのプラグインとして使用することでさらに威力を発揮します。
...
```js title="template.test.js"
const yaml = require("js-yaml");
const fs = require("fs");
const yamlCfn = require("yaml-cfn"); // CloudFormation の Yaml スキーマをサポート
...
beforeAll(async () => {
  ...
  // YAMLファイルをJavaScriptオブジェクトとして変換
  template = yaml.safeLoad(input, { schma: yanlCfn.schma });
});
...
上記のテストを CI パイプラインで実践するならば以下のようになります。Lint 系のテストは並列で実施して良いでしょう。

<div style={{ textAlign: "center" }}>
  <img alt="Docusaurus with Keytar" src={useBaseUrl("img/testing.svg")} />
</div>

別リージョンへのデプロイはある程度時間がかかるので、開発用の feature ブランチで毎回実行する/しないは、開発チームの方針に合わせて実践しましょう。

例えば

- master ブランチにマージすると プロダクション環境にデプロイされる。
- develop ブランチにマージすると Taskcat が別リージョンに環境を作成し、問題がなければ削除する

のようなブランチ戦略をとっても良いでしょうし、もっと慎重にプロダクション環境への適用を検討したいのであれば

- master ブランチにマージすると プロダクション環境にデプロイされる。
- develop ブランチにマージすると テスト環境にデプロイされる。
- feature ブランチ（個人の開発用）にコミットがあると、Taskcat が別リージョンに環境を作成し問題がなければ削除する

としても良いかと思います。開発組織に合った最適なブランチ戦略とテストを検討して下さい。
